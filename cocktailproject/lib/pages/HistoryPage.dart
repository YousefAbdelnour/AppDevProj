...
class _HistoryPageState extends State<HistoryPage> {
  ...
  Future<List<Cocktail>> loadDrinkToList() async {
    List<Cocktail> list = [];
    try {
      for (var i = 0; i < sessionManager.currentUser!.recentlyViewedDrinks.length; i++) {
        var cocktailData = await api.fetchCocktailDetailsById(int.parse(sessionManager.currentUser!.recentlyViewedDrinks[i]));
